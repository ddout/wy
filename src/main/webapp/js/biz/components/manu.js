...
Vue.component('comp-manu', {
	...
	methods: {
		...
		dataModalSubmit:function(event){
			...
			var data = {
				id:_this.Manu.id,
				name:_this.Manu.name,
				note:_this.Manu.note
			};
			this.$parent.post({
				url : url,
				data : data,
			...
			<div class="col-md-12">\
				<form class="form-inline" v-on:submit.prevent="loadData" method="post">\
					<div class="form-group">\
				    	<label>名称</label>\
				    	<input type="text" class="form-control" placeholder="生产厂家名称" v-model="search.name">\
				  	</div>\
			  ...
			  <div class="modal-dialog" role="document">\
			    <div class="modal-content">\
			      <div class="modal-header">\
			        ...
			        <h4 class="modal-title">生产厂家详情</h4>\
			      </div>\
			      <div class="modal-body">\
						<div class="form-group">\
							<label>名称</label>\
						    <input class="form-control" placeholder="名称" v-model="Manu.name"/>\
						</div>\
						<div class="form-group">\
					     	<label>备注</label>\
					    	<input class="form-control" placeholder="备注" v-model="Manu.note"/>\
						</div>\
			      </div>\
			      <div class="modal-footer">\
			      	<span style="color:red;margin-right:30px;">{{Manu.errorMsg}}</span>\
			        <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>\
			        <button type="submit" class="btn btn-primary">保存</button>\
					<input type="hidden" v-model="Manu.id"/>\
			      </div>\
			    </div>\
			  </div>\
			</form>\
