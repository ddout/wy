 ...
 * 
 */
package com.ddout.fb.utils.servlet;
...
public class RandCodeImageServlet extends HttpServlet {
    ...
    private static String randCodeType = "5";
    private static int randCodeLength = 4;

    @Override
    public void init() throws ServletException {
	super.init();
	randCodeType = this.getInitParameter("RandCodeType");
	randCodeLength = Integer.parseInt(this.getInitParameter("RandCodeLength"));
    }
	...
	// 将认证码存入SESSION
	request.getSession().setAttribute(AccessController.SESSION_KEY_OF_RAND_CODE, resultCode);
	// 图象生效
	graphics.dispose();
